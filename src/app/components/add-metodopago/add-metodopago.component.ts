...
export class AddMetodoPagoComponent {
  ...
  onSubmit(form?: any): void {
    ...
    if (this.metodoPago.comision < 0) {
      this.showAlert('error', 'La comisión no puede ser negativa.');
      return;
    }
    ...
    this._metodoPagoService.addMetodoPago(metodoPagoData).subscribe({
      next: (response: any) => {
        if (response && response.status === 201) {
          this.showAlert('success', 'Método de pago creado correctamente', () => {
            this._router.navigate(['/show-metodopago']);
          });
        } else {
